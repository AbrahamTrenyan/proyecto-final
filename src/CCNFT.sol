...
contract CCNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function buy(uint256 value, uint256 amount) external nonReentrant {
        require(canBuy, "Can't buy"); // Verificación de permisos de la compra con "canBuy". Incluir un mensaje de falla.
    ...
    function claim(uint256[] calldata listTokenIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < listTokenIds.length; i++) {
            ...
            tokenSale = tokensOnSale[listTokenIds[i]]; // Acceso a la información de venta del token
            tokenSale.onSale = false; // Desactivacion del estado de venta.
            tokenSale.price = 0; // Desactivacion del estado de venta.

            removeFromArray(listTokensOnSale, listTokenIds[i]); // Remover el token de la lista de tokens en venta.
            _burn(listTokenIds[i]); // Quemar el token, eliminándolo permanentemente de la circulación.
            emit Claim(_msgSender(), listTokenIds[i]); // Registrar el ID y propietario del token reclamado.
        }
    ...
    function trade(uint256 tokenId) external nonReentrant {
        ...
        require(canTrade, "Can't trade"); // Verificación del comercio de NFTs (canTrade). Incluir un mensaje de falla.
        require(_exists(tokenId), "Token not exists"); // Verificación de existencia del tokenId (_exists). Incluir un mensaje de falla.
    ...
    function putOnSale(uint256 tokenId, uint256 price) external {
        ...
        require(canTrade, "Can't trade"); // Verificación de operaciones de comercio (canTrade). Incluir un mensaje de falla.

        require(_exists(tokenId), "Token not exists"); // Verificción de existencia del tokenId mediante "_exists". Incluir un mensaje de falla.

        require(ownerOf(tokenId) == _msgSender(), "Only owner can put on sale"); // Verificción remitente de la transacción es propietario del token. Incluir un mensaje de falla.
    ...
    function setFundsCollector(address _address) external onlyOwner {
        ...
        require(_address != address(0), "Invalid address"); // La dirección no puede ser la dirección cero (address(0))
